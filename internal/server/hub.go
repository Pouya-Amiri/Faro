...
func (h *hub) move(p *participant, request protocol.RoomMove) (joinResult, error) {
	roomID, err := validateText("room", request.Room, maxRoomRunes)
	if err != nil {
		return joinResult{}, err
	}
	h.mu.Lock()
	defer h.mu.Unlock()
	old := h.roomForLocked(p)
	if old == nil {
		return joinResult{}, invalid("not_joined", "the participant is not in a room")
	}
	target := h.rooms[roomID]
	ownerToken := ""
	if target == nil {
		if len(h.rooms) >= h.maxRooms {
			return joinResult{}, invalid("server_full", "the server has reached its active room limit")
		...
		target = newRoom(roomID, sha256.Sum256([]byte(ownerToken)))
		h.rooms[roomID] = target
	}
	if len(target.participants) >= h.maxRoomParticipants {
		return joinResult{}, invalid("room_full", "the room has reached its participant limit")
	}
	role := protocol.RoleMember
	if ownerToken != "" {
		role = protocol.RoleOwner
	} else if request.OwnerToken != "" {
		digest := sha256.Sum256([]byte(request.OwnerToken))
		if !hmac.Equal(digest[:], target.ownerDigest[:]) {
			return joinResult{}, invalid("invalid_owner_token", "the room owner capability is invalid")
		}
		role = protocol.RoleOwner
	}
	var cancelled *protocol.PlaylistWheel
	if old.wheel != nil && old.wheel.RequesterID == p.state.ID {
		cancelled = cancelWheelLocked(old, "The person who spun the wheel changed rooms")
	}
	delete(old.participants, p.state.ID)
	revocations := removeParticipantStreamsLocked(old, p, "participant changed rooms")
	oldRecipients := sessionsOf(old)
	oldParticipants := participantsOf(old)
	oldStreamRecipients := streamSessionsOf(old)
	oldStreamOffers := streamOffersOf(old)
	if len(old.participants) == 0 {
		delete(h.rooms, old.state.ID)
	}
	p.state.Role = role
	p.state.Ready = false
	p.state.Media = nil
	p.state.Name = freeName(p.state.Name, target.participants)
	target.participants[p.state.ID] = p
	go func() {
		if cancelled != nil {
			broadcast(oldRecipients, protocol.TypePlaylistWheelUpdated, *cancelled)
		}
		broadcast(oldRecipients, protocol.TypeParticipantsUpdated, oldParticipants)
		broadcast(oldStreamRecipients, protocol.TypeStreamOffersUpdated, oldStreamOffers)
		sendRevocations(revocations)
	}()
	return joinResult{participant: p, room: target, ownerToken: ownerToken, existing: sessionsOf(target)}, nil
}
